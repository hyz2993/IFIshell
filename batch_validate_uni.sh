...
echo "This shell script is written by Yazhou He./n
	...
	# auto relocate - if disable here, use cd to relocate manually
	#local_path=$(dirname $0)
	#cd $local_path
	...
	echo -e "---\ncheck wav md5"
	sleep 2
	for dir in */Wav/;do      echo "object $dir processing"     cat "$dir"*.md5    md5 "$dir"*.wav    echo "-";
	done
	...
	sleep 2
	for dir in *;
	do    echo "object $dir processing"    mv "$dir"Wav/*.wav "$dir"Scan/    echo "-";
	done

	echo -e "---\nmove checksum to parent folder"
	sleep 2
	for dir in */;
	do    echo "object $dir processing"    mv "$dir"*/CHECKSUM.md5 "$dir"    echo "-";
	done

	#echo -e "---\nrename manifest"
	#sleep 2
	#for dir in */Scan/;
	#do    #res1="${dir: -10}"    #res="${res1::9}"    #path="${dir%%$res*}"    #echo "object $path processing"    #echo "resoluation: $res"    #mv "$path"CHECKSUM.md5 "$path""$res"_manifest.md5    #echo "-";
	#done

	echo -e "---\nchange structure"
	sleep 2
	for dir in */;
	do    echo "object $dir processing"    filename="$dir"CHECKSUM.md5    #res1=${filename%%_manifest*}    #res=${res1: -9}    echo "$filename has found generating copy"    cat $filename | tr "[:upper:]" "[:lower:]" > "$dir"CHECKSUM2.md5    sed -i "" 's/\*/ Scan\//' "$dir"CHECKSUM2.md5    echo "-";
	done
