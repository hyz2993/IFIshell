#!C:\Users\Ingest_1\AppData\Local\Programs\Python\Python310\python.exe
...
def short_test(images):
    ...
    # - Copy first 24 frames to temp directory
    ...
    print(('-*-* run 24 rawcooked *-*'))
    # rawcooked 24 frames to temp/mkv
    print("***rawcooked 24dpx...")
    # subprocess.call(['rawcooked', '-y', '--check-padding', '--output-version', '2', temp_dir, '-c:a', 'copy', '-o', mkv_file])
    subprocess.call(['rawcooked', '--check-padding', temp_dir, '-o', mkv_file])
    print("***rawcooked 24dpx completed")
...
def run_loop(args):
    '''
    This will only process one sequence. Batch processing will need to work with batch_seq2ffv2.sh.
    '''
    ...
    if get_object_entry(args):
        object_entry = get_object_entry(args)
    else:
        object_entry = ififuncs.get_object_entry()
    log_name_source = os.path.join(
        args.o, '%s_seq2ffv2_log.log' % time.strftime("_%Y_%m_%dT%H_%M_%S")
    )
    ififuncs.generate_log(log_name_source, 'seq2ffv2.py started.')
    ififuncs.generate_log(
        log_name_source,
        'eventDetail=seq2ffv2.py from IFIShells')
    ififuncs.generate_log(
        log_name_source,
    ...
    ififuncs.generate_log(log_name_source, 'seq2ffv2.py finished.')
...
def make_ffv1(
    ...
    print("***rawcooked full dpx...")
    # rawcooked_cmd = ['rawcooked', '-y', reel, '--check-padding', '--output-version', '2', '-c:a', 'copy', '-o', ffv1_path]
    rawcooked_cmd = ['rawcooked', reel, '--check-padding', '--output-version', '2', '-o', ffv1_path]
    print("***rawcooked full dpx completed")
    if args.framerate:
